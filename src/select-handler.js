...
export class SelectHandler {
    ...
    this.points.slice(0, this.order.indexOf("rot")).forEach((p, index) => {
      this.selection
        ...
        .on(
          ...
          getMoseDownFunc(this.order[index], this.el)
        );
    });
  }
  ...
  updatePoints () {
    ...
    const fromShapeToUiMatrix = parent
      .screenCTM()
      .inverseO()
      .multiplyO(this.el.screenCTM());

    this.orginalPoints = this.getPoints();
    this.points = this.orginalPoints.map((p) =>
      p.transform(fromShapeToUiMatrix)
    );
  }
  ...
  getPoints () {
    ...
    const y = y2 - this.height;
    const cy = y2 - this.height / 2;
